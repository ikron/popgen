...
read.genotype.data<-function(file, data.format="unphased", names=TRUE, header=TRUE, positions=TRUE) {
...
read.data <- function(file, format="haplotype", names=TRUE, positions=TRUE, header=TRUE) {
...
snp.frequency <- function(data, maf=TRUE) {
	...
	norm<-apply(data$seq !="NA", 2, sum, na.rm=T)
	if (data$type == "genotype") norm <- norm*2;

	freq<-ct/norm;

	if (maf==TRUE) {
		for (i in 1:data$l.seq) {
			freq[i]<-min(freq[i], 1-freq[i]);
		}
	}
	return(freq)
}
...
subset.data <- function(data, maf=-1, pos.lim=c(), select.snp=c(), convert.to.minor=FALSE) {
	...
	#On defined snps - identified by relative position
...
summarise.data <- function(data, single.plot=TRUE, ld.plot=TRUE, maf=0.0) {
	...
	cat(paste("Number of sequences = ",data$n.seq,"\n",sep=""));
	...
	#Need to fix this part - doesn't do missing data yet
	if (data$type=="haplotype") {n.sing <- length(freq[freq == 1/data$n.seq]);}
	if (data$type=="genotype") {n.sing <- length(freq[freq == 1/(2*data$n.seq)]);}
	...
	if (single.plot == FALSE) {x11();}
        hist(freq,xlim=c(0,0.5), xlab="Minor Allele Frequency", ylab="Count", col="blue", main="Minor Allele Frequency");
...
makeld<-function(data,cutoff=0,stat=c("r2","dprime")){
  freq=vector(length=ncol(data))
  for(i in 1:ncol(data)){
    freq[i]=sum(data[2:nrow(data),i]==1)/(nrow(data-1));
    if(freq[i]>0.5) freq[i]=1-freq[i]
  }
  if (length(freq>cutoff)<2) {
	cat("\n\n*** Error: Too few sites meet MAF>5% for LD plot (<2) ***\n\n");
	return();
  }
  data=data[,freq>cutoff]
  freq=freq[freq>cutoff]

  cor=array(0, dim=c(ncol(data),ncol(data)))
  
  if(length(grep("r2",stat))>0){
    for(i in 1:(nrow(cor)-1)){

      for(j in (i+1):ncol(cor)){
        ##upper diagonal
        cor[i,j]=cor(data[2:nrow(data),i],data[2:nrow(data),j])
        cor[i,j]=cor[i,j]^2
      }
    }
  }
  ...
  if(length(grep("dprime",stat))>0){
    for(i in 1:(nrow(cor)-1)){

      for(j in (i+1):ncol(cor)){
        ##lower diagonal
        cor[j,i]=cor(data[2:nrow(data),i],data[2:nrow(data),j])*sqrt(freq[i]*freq[j]*(1-freq[i])*(1-freq[j]))
        ###now d; so
        if(cor[j,i]>=0) cor[j,i]=cor[j,i]/min(freq[i]*(1-freq[j]),(1-freq[i])*freq[j])
        else cor[j,i]=-cor[j,i]/min(freq[i]*freq[j],(1-freq[i])*(1-freq[j]))
      }
    }

    min.rec = vector(length=ncol(cor));
    min.rec[1]=0;
    if (cor[2,1]<0.999) {min.rec[2]=1;}
    else {min.rec[2]=0;}

    for (k in 3:ncol(cor)) {
	min.rec[k]=0;
	for (i in 2:(k-1)) {
		if (cor[k,i]<0.999) {r.add=1;}
		else {r.add=0;}
...
plotld=function(data,cutoff=0,stat=c("r2","dprime"),left=-1,right=-1,cor=c(), col=c(), breaks=c()){

  data<-rbind(data$pos, data$seq);

  cat("\n\nCalculating LD matrix");
  if(left==-1) left=data[1,1]
  if(right==-1) right=data[1,ncol(data)]
  if(length(cor)>0){
    cor=cor[,(data[1,]<=right & data[1,]>=left)]
    cor=cor[(data[1,]<=right & data[1,]>=left),]
  }
  data=data[,data[1,]<=right & data[1,]>=left]
  ...
  if (length(breaks)==0) {
	image(data[1,],data[1,],cor,zlim=c(0,1.02),xlab="Position",ylab="Position", 
...
summarise.interval=function(rates.file="rates.txt", burn.in=30, locs.file=FALSE) {
  ...
  x11();
  par(mfrow=c(1,2));
  plot(x[,1], type="l", col=rgb(0,0,0.5), xlab="Sample", ylab="Total map length", 
  ...
  means<-apply(x[low:nrow(x),], 2, mean);
  q.95<-apply(x[low:nrow(x),], 2, quantile, probs=c(0.025, 0.5, 0.975));
  ...
  plot(pos[1:(length(pos)-1)], y=means[2:length(means)], type="l", col=rgb(0,0,0.5), 	
	xlab=xlab, ylab="Posterior mean rate", main="Posterior mean rates");
  lines(pos[1:(length(pos)-1)], y=q.95[1,2:length(means)], type="l", col=grey(0.75), lty="dotted");
  lines(pos[1:(length(pos)-1)], y=q.95[3,2:length(means)], type="l", col=grey(0.75), lty="dotted");
...
write.ldhat.format = function(x, seq.file="seqs.txt", loc.file="locs.txt") {
	...
	write(paste(x$n.seq, x$l.seq, hd, sep=" "), seq.file);
	for (i in 1:x$n.seq) {
		write (paste(">", x$names[i], sep=""), seq.file, append=TRUE);
		write(x$seq[i,], seq.file, append=TRUE, ncolumns=x$l.seq);
	}
...
plot.tree<-function(data, maxdepth=-1, add.mutations=FALSE, y.low=0, names.plot=TRUE, lwd=1, cex.mtn=0.5, plot.order=c()) {
...
#DO RH vertical
	...
	if (names.plot==TRUE) {
		text(x=c(1:nseq), y=rep(0, nseq), labels=data[plot.order,1], srt=90, cex=0.5, pos=1)
	}
	return(plot.order)
}
...
#			cat(paste("\nCoalescent between lineages ", l1, " and ", l2,sep=""));
	...
	if (plot.tree == TRUE) {
		...
		pos<-ceiling(c(1:n.tree)*sites/n.tree);
		mx<-max(nodes[pos,,2]);
		for (tree in 1:n.tree) plot.tree(nodes[pos[tree],,], maxdepth=mx, add.m=TRUE);
		par(mfrow=c(1,1));
	}
